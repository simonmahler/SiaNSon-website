...
<script language=\"JavaScript\" src=\"$menuVars[pathToCodeFiles]$menuVars[file_jsapi]\" type=\"text/javascript\"></script>
...
function drawMenus()
{
	...
	if($drawCount==0)
	{
		...
		{
			if($k!="pathToCodeFiles"&&$k!="file_milonicsrc"&&$k!="file_mmenudom"&&$k!="file_mmenuns4"&&$k!="file_jsapi")
			{
				echo "_$k=$v;\n";
			}
		}
...
function buildMySQLMainMenu($menuProjectID)
{
	global $table_prefix, $menuVars, $dbHost, $dbUser, $dbPasswd, $dbName;
	...
	$query="select * from ".$table_prefix."menus where projectid = $menuProjectID and alwaysvisible=1";
	$mqry=runquery($query);
